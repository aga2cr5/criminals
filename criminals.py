...
def count_criminals(emails):
    #count and return the amount of criminals in the emails

    # REGEX PATTERNS
    co_uk = re.compile(r'[a-zA-Z0-9_.+-]+@[a-zA-Z0-9-]+\.co.uk')
    wonk_grin = re.compile(r'[a-zA-Z0-9_.+-]+@(wonkaindustries|gringottsbank)+\.[a-zA-Z0-9-.]+')


    # CO.UK ARE CRIMINALS
    uk_criminals = list(filter(co_uk.match, emails))
    full_names = [name.split("@")[0] for name in uk_criminals]
    criminals = [name for name in full_names if "c" in name]

    # WONKAINDUSTRIES AND GRINGOTTSBANK ARE CRIMINALS
    wonk_grin = list(filter(wonk_grin.match, emails))
    full_names2 = [name.split("@")[0] for name in wonk_grin]
    for name in full_names2:
      first_name = name.split(".")[0]
      if len(first_name) >= 4:
        criminals.append(name)

    # CRIMINAL IF TWO OR MORE LETTERS T IN FULL NAME
    all_names = [name.split("@")[0] for name in emails]
    for name in all_names:
      if name.count("t") >= 2:
        criminals.append(name)

    # INNOCENT IF R IN LAST NAME
    for name in all_names:
      last_name = name.split(".")[1]
      if "r" in last_name:
        if name in criminals:
          criminals.remove(name) 

    # REMOVING POSSIBLE DUPLICATE NAMES
    count = len(list(set(criminals)))
    return count


if __name__ == "__main__":
  count_criminals(emails)